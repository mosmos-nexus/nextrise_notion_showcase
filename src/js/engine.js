...
(function(){
  ...
  /* WP1 sub-task 4: touch — 스와이프 + 탭 내비. HOLD 슬라이드에서는 무시 */
  let tx=null, ty=null;
  addEventListener('touchstart',e=>{tx=e.touches[0].clientX; ty=e.touches[0].clientY},{passive:true});
  addEventListener('touchend',e=>{
    if(tx===null)return;
    if(document.body.classList.contains('editing')){ tx=null; ty=null; return; }
    /* HOLD: touch ignored entirely on hold slides */
    if(holdActive){ tx=null; ty=null; return; }
    ...
    if(Math.abs(dx)>56 && Math.abs(dx)>Math.abs(dy)){
      ...
      /* 탭 — 링크·버튼·진행바·라이브 탐색 영역 탭은 제외, 화면 왼쪽 1/4은 이전 그 외는 다음 */
      const skip=e.target.closest&&e.target.closest('a,button,.orbit-progress,.explore-hint,.edit-toggle,.edit-hotzone,[data-explore].explore,iframe');
      if(!skip) (t.clientX<innerWidth*0.25)?prevSlide():next();
    }
    tx=null; ty=null;
  },{passive:true});
